...
/**
 * Support class for the Even Realities G1. This sends and receives commands to and from the device. The Protocol is
 * mostly defined in G1Constants.java right now. In the future the protocol will be broken out to a different class.
 * One interesting point about this device is that it requires a constant BLE connection which is contrary to the way
 * BLE is supposed to work. Unfortunately the device will show the disconnected icon and stop displaying any information
 * when it is in the disconnected state. Because of this, we need to send a heartbeat ever 30 seconds, otherwise the
 * device will disconnect and reconnect every 32 seconds per the BLE spec.
 */
public class G1DeviceSupport extends AbstractBTLEDeviceSupport {
  ...
  protected TransactionBuilder initializeDevice(TransactionBuilder builder) {//}, int deviceIdx) {
    ...
    if (rxCharacteristic == null || txCharacteristic == null) {
      ...
      GB.toast(getContext(), "Failed to connect to Glasses, waiting for reconnect.", Toast.LENGTH_LONG, GB.ERROR);
      return builder;
    }
